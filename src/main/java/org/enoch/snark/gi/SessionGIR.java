    ...
    public boolean signInWithRetry() {
        for (int i = 0; i < 3; i++) {
            try {
                ...
                gi.findElement(TAG_INPUT, ID_ATTRIBUTE, LOGIN_INPUT).sendKeys(Instance.getGlobalMainConfigMap().getConfig(ThreadMap.LOGIN));
                gi.findElement(TAG_BUTTON, ID_ATTRIBUTE, LOGIN_BUTTON).click();
                SleepUtil.sleep();

                gi.findElement(TAG_INPUT, ID_ATTRIBUTE, PASSWORD_INPUT).sendKeys(Instance.getGlobalMainConfigMap().getConfig(ThreadMap.PASSWORD));
    ...
    public boolean openServer() {
        for (int i = 0; i < 3; i++) {
            try {
                ...
                String serverName = Instance.getGlobalMainConfigMap().getConfig(ThreadMap.SERVER);
