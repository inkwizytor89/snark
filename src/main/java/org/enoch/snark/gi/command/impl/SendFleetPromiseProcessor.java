...
import org.enoch.snark.instance.model.exception.NoAvailableFleetSlots;
...
public class SendFleetPromiseProcessor {
    ...
    public boolean execute(SendFleetPromiseCommand command) {
        FleetPromise promise = command.promise;
        GIUrl.openSendFleetView(command.promise.getSource(), command.promise.getTarget(), command.promise.getMission());
        validate();
        //Scroll down till the bottom of the page
        ((JavascriptExecutor) gir.getWd()).executeScript("window.scrollBy(0,document.body.scrollHeight)");


        if(isAllShips() && (promise().getLeaveShipsMap() == null || promise().getLeaveShipsMap().isEmpty())) {
            gir.selectAllShips();
        } else if (promise().getShipsMap() != null) {
            promise().setSource(command.promise.getSource());
            ...
            for (Map.Entry<Ship, Long> entry : realCanToSend.entrySet()) {
                ...
                command.promise.setShipsMap(ShipsMap.createSingle(entry.getKey(), value));
        ...
        try {
            ...
            System.err.println("promise required on planet "+command.promise.getSource());
            throw e;
        }

        gir.setSpeed(command.promise.getSpeed());
        gir.setNewResources(command.promise);

        FleetEntity fleet = new FleetEntity(command.promise);
        fleet.hash = command.hash();
        ...
        if(gir.getWd().findElements(By.className("status_abbr_noob")).size() != 0) {//player is green - too weak
